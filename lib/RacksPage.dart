...
class Rackspage extends StatefulWidget {
  const Rackspage({super.key});

  @override
  State<Rackspage> createState() => _RackspageState();
}

class _RackspageState extends State<Rackspage> 
    with TickerProviderStateMixin {
  List<dynamic> racksProducts = [];
  ...
  final List<String> categories = ['All','racks', 'squat racks', 'power racks', 'cage racks'];
  ...
  void loadData() async {
    try {
      List<dynamic> data = await loadracksData();
      setState(() {
        racksProducts = data;
  ...
  Future<List<dynamic>> loadracksData() async {
    try {
      String jsonString = await rootBundle.loadString('assets/rack.json');
      List<dynamic> jsonData = json.decode(jsonString);
      return jsonData;
    } catch (e) {
      print('Error loading racks data: $e');
      throw e;
    }
  }
  ...
  void _filterProducts() {
    setState(() {
      filteredProducts = racksProducts.where((product) {
          ...
          children: [
            ...
            Text(
              searchQuery.isNotEmpty 
                  ...
                  : 'Please check if rack.json exists in assets folder',
              textAlign: TextAlign.center,
              style: TextStyle(
                fontSize: 16,
